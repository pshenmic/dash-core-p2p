 ...
 */
export function validateTransactionForBroadcast(
  tx: Transaction,
  opts?: { maxTxSize?: number },
): void {
  ...
  try {
    ...
    throw new Error('broadcast: failed to serialize transaction: ' + (e as Error).message);
  }
  if (bytes.length === 0) {
  ...
  if (bytes.length > max) {
    throw new Error(
      `broadcast: transaction is ${bytes.length} bytes, over the ${max}-byte standard relay limit`,
    );
  }
  ...
  try {
    ...
    throw new Error('broadcast: failed to compute txid: ' + (e as Error).message);
  }
...
export class TxBroadcast extends EventEmitter {
  ...
  readonly invSentTo: Set<Peer> = new Set();
  /** Peers we have already sent the `tx` message to. */
  readonly txSentTo: Set<Peer> = new Set();
  /** Peers that issued `getdata` for our txid. */
  readonly requestedBy: Set<Peer> = new Set();
  /**
  ...
  private readonly _txidWireHex: string;
  /**
  ...
  readyPeers(): Peer[] {
    const map = (this.pool as unknown as { _connectedPeers: Record<string, Peer> })._connectedPeers ?? {};
    const out: Peer[] = [];
    for (const p of Object.values(map)) {
      if (p && p.status === PeerStatus.READY) out.push(p);
    }
    return out;
  }
  ...
  announce(peer?: Peer): Peer[] {
    ...
    for (const p of targets) {
      if (this.invSentTo.has(p)) continue;
      if (p.status !== PeerStatus.READY) continue;
      try {
  ...
  push(peer: Peer): boolean {
    this._assertOpen();
    if (peer.status !== PeerStatus.READY) return false;
    if (this.txSentTo.has(peer)) return false;
    try {
      const txMsg = (this._messages as unknown as { Transaction: (tx: Transaction) => Message })
        .Transaction(this.tx);
  ...
  private _assertOpen(): void {
    if (this._closed) throw new Error('TxBroadcast: session is closed');
  }

  private _buildInv(): Message {
    const factory = (this._messages as unknown as {
      Inventory: (items: InventoryItem[]) => Message;
    }).Inventory;
    return factory([{ type: InventoryType.TX, hash: this.txidWire }]);
  }

  private _matchesTxid(item: InventoryItem): boolean {
    if (!isTxInvType(item.type)) return false;
    if (!(item.hash instanceof Uint8Array) || item.hash.length !== 32) return false;
    return bytesEqual(item.hash, this.txidWire);
  }

  private _attach(): void {
    const onGetData = (peer: Peer, msg: Message & { inventory?: InventoryItem[] }) => {
      const inv = msg.inventory ?? [];
      let matched = false;
      for (const item of inv) {
        if (this._matchesTxid(item)) { matched = true; break; }
      }
      if (!matched) return;
    ...
    const onInv = (peer: Peer, msg: Message & { inventory?: InventoryItem[] }) => {
      const inv = msg.inventory ?? [];
      for (const item of inv) {
    ...
    const onIslock = (_peer: Peer, msg: Message & ISLockArgs) => {
    ...
    const onReject = (
      ...
      if ((msg.message ?? '').toLowerCase() !== 'tx') return;
      if (!msg.data || msg.data.length < 32) return;
      const hash = msg.data.subarray(0, 32);
      if (!bytesEqual(hash, this.txidWire)) return;
      const info: RejectInfo = {
        peer,
        ccode: msg.ccode ?? 0,
        reason: msg.reason ?? '',
      };
...
function isTxInvType(type: number): boolean {
