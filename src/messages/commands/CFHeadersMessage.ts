...
export class CFHeadersMessage extends Message {
  ...
  setPayload(payload: Uint8Array): void {
    ...
    this.filterHashes = [];
    for (let i = 0; i < count; i++) this.filterHashes.push(parser.read(32).slice());
    utils.checkFinished(parser);
  }

  getPayload(): Uint8Array {
    if (this.stopHash.length !== 32) throw new Error('stopHash must be 32 bytes');
    if (this.previousFilterHeader.length !== 32) throw new Error('previousFilterHeader must be 32 bytes');
    ...
    for (const h of this.filterHashes) {
      if (h.length !== 32) throw new Error('filterHash must be 32 bytes');
      bw.write(h);
    }
    return bw.concat();
  }
