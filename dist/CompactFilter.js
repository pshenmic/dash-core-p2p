...
class BitReader {
    ...
    readBit() {
        ...
        if (i >= this.buf.length)
            throw new Error('GCS bit reader past end of payload');
...
export function decodeGCS(filter, P = GCS_P) {
    ...
    for (let i = 0; i < N; i++) {
        ...
        while (bits.readBit() === 1) {
            q++;
            if (q > 1 << 20)
                throw new Error('GCS quotient runaway');
        }
...
export function deriveFilterKey(blockHashWire) {
    if (blockHashWire.length !== 32)
        throw new Error('blockHash must be 32 bytes (internal byte order)');
...
export function nextFilterHeader(filterBytes, prevHeader) {
    if (prevHeader.length !== 32)
        throw new Error('prevHeader must be 32 bytes');
...
//# sourceMappingURL=CompactFilter.js.map